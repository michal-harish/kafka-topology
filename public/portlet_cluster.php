<?php
    ...
    $connectionString = $query[0];
    if (isset($query[1])) {
        $consumerGroup = $query[1];
    }
    $zk = new ZooKeeper($connectionString);

    if (isset($consumerGroup)) {
        $consumers =   @$zk->exists("/consumers/$consumerGroup") ? array($consumerGroup=>array()) : array();
        $topicNames = @$zk->getChildren("/consumers/$consumerGroup/offsets"); 
    } else {
        $consumers =   @$zk->exists('/consumers') ? array_fill_keys(@$zk->getChildren('/consumers'), array()) : array();
        $topicNames = $topicNames = @$zk->getChildren('/brokers/topics');
        class watcher {
